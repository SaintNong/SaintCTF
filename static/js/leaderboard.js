...
$(document).ready(function () {
    ...
    leaderboardEvents.addEventListener("recentActivity", (event) => {
        ...
        $.each(data, function (index, solve) {
            ...
            tbody.append(`<tr ${highlight}><td><a href="/profile/${solve.solver_id}">${solve.solver}</a></td><td${first_blood}</td><td><a href="/challenges#chall-${solve.challenge_id}">${solve.challenge.name}</a></td><td><time datetime="${solve.time}">${relative_time}</time></td></tr>`);
        });
