...
def get_default_config(app):
    ...
    # Generate default xkcd style passphrase
    ...
    default_config = {
        ...
        "ADMIN_USERNAME": {
            "default": "Admin",
            "comment": "Default username for the admin account",
        },
        "ADMIN_PASSWORD": {
            "default": password,
            "comment": "Default password for the admin account",
        },
...
def create_app():
    ...
    with app.app_context():
        ...
        admin = User.query.filter(User.admin == True).first()
        if admin:
            app.logger.warning(f"Admin account '{admin.username}' already exists.")
        else:
            # Create a Defaul admin account
            hashed_password = bcrypt.generate_password_hash(app.config["ADMIN_PASSWORD"])
            admin = User()
            admin.username = app.config["ADMIN_USERNAME"]
            admin.password = hashed_password
            admin.admin = True
            db.session.add(admin)
            app.logger.info(f"Created admin account '{admin.username}' with password '{app.config['ADMIN_PASSWORD']}'")
